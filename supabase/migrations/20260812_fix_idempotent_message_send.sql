...
begin
  ...
  with inserted_message as (
    ...
    where public.messages.client_attempt_id is not null
    do nothing
    returning
      public.messages.id,
      public.messages.conversation_id,
  ...
  select
    resolved_message.id,
    resolved_message.conversation_id,
    resolved_message.sender_id,
    resolved_message.body,
    resolved_message.created_at,
    resolved_message.client_attempt_id
  from resolved_message
  order by resolved_message.created_at asc, resolved_message.id asc
  limit 1;
end;
$$;
